...
public class memberLoginController extends HttpServlet {
	...
	protected void service(HttpServletRequest request, HttpServletResponse response)
		...
		PrintWriter writer = response.getWriter(); // jsp => out 객체 
		if (dbPw == null || dbPw.equals(pw) == false) {
			writer.println("<script>alert('로그인 실패 '); location.href='member/memberLogin.jsp;</script>");
		} else {
			HttpSession session = request.getSession(); // jsp -> session객체 불러오기 
			session.setAttribute("log", MemberDAO.getInstance().getMemberNo(id));
			writer.println("<script>alert('" + id + "님 로그인 성공 '); location.href='"+ctx+"/memberList.do';</script>");
		}
